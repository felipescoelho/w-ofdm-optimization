...
% Set 30, 2022
%
% Modified by F. Cruz - fernando.cruz@uah.es
% May 16, 2023
...
%     'bitsPerSubcarrier', 4, 'cyclicPrefix', 18:2:32, 'symbolsPerTx', ...
...
% - Following line modified by fcruz
                optimize_window_new(fieldName, cpLength, csLength, ...
...
function optimize_window_new(typeOFDM, cpLength, csLength, numSubcar, ...
    tailTx, tailRx, prefixRemovalLength, circularShiftLength, alpha)
% Function to generate the optimized windows.
%
% - Input:
...
%             alpha);
        initialValue = diag(windowTxRC);
% - The following constraint is not needed since it can be controlled by lower- upper-bounds (fcruz)
%         Aeq = [zeros(numSubcar+cpLength+csLength-2*tailTx, tailTx), ...
%             eye(numSubcar+cpLength+csLength-2*tailTx), ...
%             zeros(numSubcar+cpLength+csLength-2*tailTx, tailTx)];
%         beq = ones(numSubcar+cpLength+csLength-2*tailTx, 1);

% -This constraint is to enforce the symmetry of the window tails (fcruz)
        Aeq = [eye(tailTx) zeros(tailTx,numSubcar+cpLength+csLength-2*tailTx) -fliplr(eye(tailTx))];
        beq = zeros(tailTx,1);

% -The following constraint is to ensure that vtr[n] is an increasing function while vtf is decreasing (fcruz)
%         RA = [eye(tailTx-1) zeros(tailTx-1,1)]+[zeros(tailTx-1,1) eye(tailTx-1)];
%         RB = [eye(tailTx-1) zeros(tailTx-1,1)]+[zeros(tailTx-1,1) +eye(tailTx-1)];
%         A1 = [RA zeros(tailTx-1,numSubcar+cpLength+csLength-tailTx)];
%         A2 = [zeros(tailTx-1,numSubcar+cpLength+csLength-tailTx) RB];
%         A = [A1; A2];
%         b = -0.01*ones(2*tailTx-2, 1);
% -The following constraint is to ensure that vtr[beta]-vtr[1] > 0.5 (fcruz)
%         Aa = [1 zeros(1,tailTx-2) -1 zeros(1,numSubcar+cpLength+csLength-tailTx)];
%         Ab = [zeros(1,numSubcar+cpLength+csLength-tailTx) -1 zeros(1,tailTx-2) 1 ];
%         A=[A1; A2; Aa; Ab];
%         b2 = -0.5*ones(2,1);
%         b1 = -0.01*ones(2*tailTx-2, 1);
%         b=[b1; b2];
         A = [];
         b = [];
% -The following sentences force that vtr[1], vtf[beta] < 0.2 and vtr[beta], vtf[1] > 0.8 (fcruz)
%         lowerBounds = [0.01*ones(tailTx-1, 1);...
%             0.8;...
%             ones(numSubcar+cpLength+csLength-2*tailTx, 1);
%             0.8;...
%             0.01*ones(tailTx-1, 1)];
%         upperBounds = [0.2;...
%             0.99*ones(tailTx-1, 1); ...
%             ones(numSubcar+cpLength+csLength-2*tailTx, 1); ...
%             0.99*ones(tailTx-1, 1);...
%             0.2];
% -The following sentences force that 0.01 <= vtr[n], vtf[n] <= 0.99 (fcruz)
%         lowerBounds = [0.01*ones(tailTx, 1);...
%             ones(numSubcar+cpLength+csLength-2*tailTx, 1);
%             0.01*ones(tailTx, 1)];
%         upperBounds = [0.99*ones(tailTx, 1); ...
%             ones(numSubcar+cpLength+csLength-2*tailTx, 1); ...
%             0.99*ones(tailTx, 1)];
% -The following sentences (proposed by Luiz) force that 0 <= vtr[n], vtf[n] <= 1 (fcruz)
        lowerBounds = [zeros(tailTx, 1); ...
            ones(numSubcar+cpLength+csLength-2*tailTx, 1); ...
            zeros(tailTx, 1)];
        upperBounds = [ones(tailTx, 1); ...
        ...
        while tryCount < maxTries
            ...
            if exitFlag == 0
                [windowVector, val, exitFlag] = quadprog(HTx, ...
                    zeros(1, length(initialValue)), A, b, Aeq, beq, ...
                    lowerBounds, upperBounds, initialValue, options2);
                initialValue = windowVector;
            end
        end
...
%             intersymbolInterference, alpha);
        initialValue = diag(windowRxRC);
% -This constraint is to enforce the symmetry of the window tails vrr[n]=vrf[delta-n] (fcruz)
        Aeq1 = [eye(tailRx) zeros(tailRx,numSubcar-tailRx) -fliplr(eye(tailRx))];
        beq1 = zeros(tailRx,1);
% -This constraint programmed by Luiz is to enforce that vrr[n]+vrf[delta-n] = 1 (fcruz)
        Aeq2 = [eye(tailRx), zeros(tailRx, numSubcar-tailRx), eye(tailRx)];
        beq2 = ones(tailRx, 1);
% - Luiz proposal (fcruz)
        Aeq = Aeq2;
        beq = beq2;
...
% - The following constraint is not needed since it can be controlled by lower- upper-bounds (fcruz)
        ...
        beqTx = ones(numSubcar+cpLength+csLength-2*tailTx, 1);
% -This constraint is to enforce the symmetry of the window tails (fcruz)
        AeqTx = [eye(tailTx) zeros(tailTx,numSubcar+cpLength+csLength-2*tailTx) -fliplr(eye(tailTx))];
        beqTx = zeros(tailTx,1);
...
% -This constraint is to enforce the symmetry of the window tails vrr[n]=vrf[delta-n] (fcruz)
        AeqRx1 = [eye(tailRx) zeros(tailRx,numSubcar-tailRx) -fliplr(eye(tailRx))];
        beqRx1 = zeros(tailRx,1);
% -This constraint programmed by Luiz is to enforce that vrr[n]+vrf[delta-n] = 1 (fcruz)
        AeqRx2 = [eye(tailRx), zeros(tailRx, numSubcar-tailRx), eye(tailRx)];
        beqRx2 = ones(tailRx, 1);
% - Luiz proposal (fcruz)
        AeqRx = AeqRx2;
        beqRx = beqRx2;
% - New proposal (fcruz)
